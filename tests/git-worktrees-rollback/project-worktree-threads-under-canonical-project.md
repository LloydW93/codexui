...
8. From the canonical project row, start a new local chat and confirm the selected folder is the base checkout, not a `~/.codex/worktrees/<id>/...` path.
9. Open the project row actions that depend on the project cwd, such as browse files or create new worktree, and confirm they target the base checkout.
